...
def callback_joystick_values(data):
    ...
    pitch_value = data.pitch -15
    yaw_value = -(data.yaw)
    rospy.loginfo("joystick value, roll: %.2f, pitch: %.2f, yaw: %.2f", data.roll, data.pitch, data.yaw)
    ...
    for dxl_id, goal_position in zip([11, 12, 13], [convert_yaw, convert_pitch, convert_roll]):
